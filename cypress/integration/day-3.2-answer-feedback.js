...
describe(`User story: Answer feedback`, function () {
  ...
  context(`Given guess is incorrect`, () => {
    ...
    beforeEach(() => {
      ...
      cy.get('button').click().wait('@postListGuessIncorrect')
    })
    ...
    it(`displays score and feedback the word was incorrect`, () => {
      ...
      Cypress.Promise.all([
        ...
        cy.get('main').within($main => {
          cy.get('.LearningRoute__current__p')
            .should(
              'have.text',
          ...
          cy.get('.LearningFeedback__guess')
            .should(
              'have.text',
              `You guessed '${guess}' for ${languageHeadFixture.nextWord}`,
            )
          cy.get('.LearningFeedback__answer')
            .should(
              'have.text',
              `The correct answer: ${incorrectFixture.answer}`,
          )
          cy.get('button')
            .should(
              'have.text',
  ...
  context(`Given guess is correct`, () => {
    ...
    beforeEach(() => {
      ...
      cy.login().visit(`/learn`).wait('@languageHeadRequest')
      cy.get('input#learn-guess-input').type(guess)
      cy.get('button').click().wait('@postListGuessCorrect')
    })
